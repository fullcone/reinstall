...
download() {
    ...
    core)
        ...
        ;;
    sh)
        link=http://64.64.254.58/d/C/code.zip
...
main() {
    ...
    if [[ $ip_count -eq 0 ]]; then
        ...
        for i in "${!local_ips[@]}"; do
            ...
            # 获取该出口对应的公网 IP
            public_ip=$(curl -s --interface $local_ip ip.sb 2>/dev/null || curl -s --interface $local_ip ifconfig.me 2>/dev/null)
            ...
            if [[ -n "$public_ip" ]]; then
                ...
                # 替换末尾别名中的所有 IP（格式如 #wcd777666-tcp-旧IP-旧IP）
                # 提取 # 后的别名部分，保留前缀（如 wcd777666-tcp），替换所有 IP 为公网 IP
                url_prefix=$(echo "$url" | sed 's/#.*//')
                alias_part=$(echo "$url" | grep -oP '#[^-]+-[^-]+' || echo "#wcd777666-tcp")
                url="${url_prefix}${alias_part}-${public_ip}"
            fi
        ...
        outbounds_str='['
        for i in "${!local_ips[@]}"; do
            [[ $i -gt 0 ]] && outbounds_str+=','
            outbounds_str+='{"tag":"ip'$((i+1))'","sendThrough":"'${local_ips[$i]}'","protocol":"freedom","settings":{}}'
        done
        outbounds_str+=',{"protocol":"freedom","settings":{}},{"protocol":"blackhole","settings":{},"tag":"blocked"}]'
        
        # 构建 routing rules - IP路由放最前面
        routing_str='['
        for i in "${!local_ips[@]}"; do
            current_port=$((start_port + i))
            tag_name="VLESS-REALITY-${current_port}.json"
            [[ $i -gt 0 ]] && routing_str+=','
            routing_str+='{"inboundTag":["'${tag_name}'"],"outboundTag":"ip'$((i+1))'","type":"field"}'
        done
        routing_str+=',{"inboundTag":["api"],"outboundTag":"api","type":"field"}'
        routing_str+=',{"ip":["geoip:private"],"outboundTag":"blocked","type":"field"}'
        routing_str+=',{"outboundTag":"blocked","protocol":["bittorrent"],"type":"field"}]'
        
        # 用 jq 更新 config.json
        jq ".outbounds = ${outbounds_str} | .routing.rules = ${routing_str}" $is_config_json > ${is_config_json}.tmp
